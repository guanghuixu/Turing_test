...
if __name__ == '__main__':
    ...
    #   refs = {'generated_description'+str(i):[x.strip()] for i,x in enumerate(f.readlines())}
    stack_num = 50
    cands_num = len(cands.keys())  # 10000

    with open('dataset/cosine_all.txt') as f:
        cosine_all = f.readlines()
    cosine_all = cosine_all[:cands_num]
    with open('dataset/unprocessed.train.json') as f:
    ...
    for i in range(stack_num):
        ...
        refs = [annotation[int(graph_id)]["abstract_og"] for graph_id in cosine_all[i].split(',')]
        # refs = [' '.join(refs)]
        score = x.evaluate(live=True, cand={key: cands[key]}, ref={key: refs})
        bleu_id.append(i)
    ...
    with open('dataset/group_baseline_1w.txt') as f:
        group_baseline = f.readlines()
    for ids in bleu_id:
        key = 'generated_description{}'.format(ids)
        with open('./results/group_top_{}.txt'.format(stack_num), 'a+') as f:
            f.writelines(cands[key])
            f.writelines('\n')
        with open('./results/group_baseline_{}.txt'.format(stack_num), 'a+') as f:
            f.writelines(group_baseline[ids])
            f.writelines('\n')
